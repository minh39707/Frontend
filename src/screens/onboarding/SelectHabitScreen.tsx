...
export default function SelectHabitScreen() {
  ...
  return (
    <ScreenContainer>
      ...
      {data.habit_name ? (
        <Animated.View entering={FadeInDown.duration(420).delay(20)} style={styles.preview}>
      ...
      <View style={styles.list}>
        {PRESET_HABITS.map((habit, index) => (
          <Animated.View entering={FadeInDown.duration(440).delay(index * 50)} key={habit.value}>
            <SelectCard
              description={habit.description}
              icon={habit.icon}
              onPress={() => setHabitSelection(habit.value, 'preset')}
              selected={data.habit_type === 'preset' && data.habit_name === habit.value}
              title={habit.label}
            />
          </Animated.View>
        ))}
      ...
      <View style={styles.actions}>
        <PrimaryButton
          disabled={!data.habit_name}
          label="Continue with this habit"
      ...
      <CustomHabitModal
        initialValue={data.habit_type === 'custom' ? data.habit_name ?? '' : ''}
