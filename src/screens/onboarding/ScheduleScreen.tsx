...
export default function ScheduleScreen() {
  ...
  if (!isOnboardingReadyForSchedule(data.habit_name)) {
    return (
      <ScreenContainer contentContainerStyle={styles.fallbackContent}>
        <EmptyStateCard
          actionLabel="Choose a habit"
          description="Your first habit has not been selected yet, so the schedule cannot be created."
          onAction={() => router.replace('/select-habit')}
          title="Pick a habit first"
        />
      </ScreenContainer>
    );
  }
  ...
  return (
    <ScreenContainer>
      ...
      <View style={styles.section}>
        ...
        <View style={styles.list}>
          {TIME_PERIOD_OPTIONS.map((option, index) => (
            <Animated.View entering={FadeInDown.duration(430).delay(index * 45)} key={option.value}>
              <SelectCard
                description={option.description}
                icon={option.icon}
                onPress={() => setTimePeriod(option.value)}
                selected={data.time_period === option.value}
                title={option.label}
              />
            </Animated.View>
          ))}
          ...
          {FREQUENCY_OPTIONS.map((option, index) => (
            <Animated.View entering={FadeInDown.duration(430).delay(index * 45)} key={option.value}>
              <SelectCard
                ...
                selected={data.frequency === option.value}
                title={option.label}
              />
            </Animated.View>
          ))}
      ...
      <PrimaryButton
        disabled={!specificDaysValid}
        label="Set My First Habit"
        onPress={() => router.push('/save')}
        style={styles.cta}
      />

      <TimePickerModal
        initialTime={data.time_exact}
