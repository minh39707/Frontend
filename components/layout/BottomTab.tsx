...
export default function BottomTab({ state, descriptors, navigation }: BottomTabBarProps) {
  ...
  const renderItem = (route: (typeof state.routes)[number], index: number) => {
    ...
    return (
      <Pressable
        ...
        <Ionicons
          name={focused ? iconMap[route.name as keyof typeof iconMap].active : iconMap[route.name as keyof typeof iconMap].idle}
          size={20}
  ...
  return (
    <View style={[styles.shell, { bottom: Math.max(insets.bottom, spacing.md) }]}>
