...
import { ResourceStat } from '@/services/user.service';
...
export default function StatsBars({ stats, compact = false }: Props) {
  return (
    <View style={styles.container}>
      {stats.map((stat) => (
        <View key={stat.label} style={[styles.row, compact && styles.rowCompact]}>
          <View style={styles.topRow}>
            <View style={styles.labelRow}>
              <View style={[styles.iconWrap, { backgroundColor: `${stat.color}14` }]}>
                <Ionicons name={stat.icon as keyof typeof Ionicons.glyphMap} size={14} color={stat.color} />
              </View>
